...
const MUTATIONS = [
  ...
  { id: "ai/citability", file: "src/checks/ai.ts",
    from: "if (missing.length >= 2 && !isCommerce) {", to: "if (false) {" },
  ...
  { id: "site/broken-links", file: "src/site/crawl.ts",
    from: "if (broken404.length) {", to: "if (false) {" },
  { id: "site/sitemap-set-scope", file: "src/site/crawl.ts",
    from: "const sitemapSet = new Set(locs.map(", to: "const sitemapSet = new Set(targets.map(" },
  { id: "site/probe-cap", file: "src/site/crawl.ts",
    from: "const probes = missing.slice(0, 25);", to: "const probes = missing.slice(0, 0);" },
  { id: "structured/aliases", file: "src/checks/structured.ts",
    from: "return filled(key) || (ALIASES[key] ?? []).some(filled);", to: "return filled(key);" },
  { id: "site/origin-from-sitemap", file: "src/site/crawl.ts",
    from: "      try { siteOrigin = new URL(sitemapUrl).origin; } catch { /* keep the requested one */ }", to: "" },
  { id: "site/resolved-sitemap-url", file: "src/site/crawl.ts",
    from: "sitemapUrl = loaded.resolvedUrl ?? candidate;", to: "sitemapUrl = candidate;" },
  { id: "sitemap/resolved-url", file: "src/site/sitemap.ts",
    from: "result.resolvedUrl = res.finalUrl;", to: "" },
  { id: "site/same-origin", file: "src/site/crawl.ts",
    from: "    .filter((l) => { try { return new URL(l).origin === siteOrigin; } catch { return false; } })", to: "" },
  ...
  { id: "cli/site-dedupe", file: "src/cli.ts",
    from: "      report.findings = report.findings.filter((f) => !covered.has(f.code));", to: "" },
  { id: "cli/site-dedupe-scope", file: "src/cli.ts",
    from: "      const covered = new Set(walked.findings.map((f) => f.code));",
    to: "      const covered = new Set(report.findings.map((f) => f.code));" },
  ...
  { id: "site/group-evidence", file: "src/site/crawl.ts",
    ...
    to: "        : urls.slice(0, 5)," },
  { id: "ai/commerce-detection", file: "src/checks/ai.ts",
    from: "  const isCommerce = !isArticle && [", to: "  const isCommerce = false && [" },
  { id: "ai/commerce-article-wins", file: "src/checks/ai.ts",
    from: "const isCommerce = !isArticle && [", to: "const isCommerce = [" },
  { id: "ai/chunk-scope", file: "src/checks/ai.ts",
    from: "if (parts.length > 1 && !isCommerce) {", to: "if (parts.length > 1) {" },
  { id: "ai/citability-scope", file: "src/checks/ai.ts",
    from: "if (missing.length >= 2 && !isCommerce) {", to: "if (missing.length >= 2) {" },
  { id: "ai/listing-suppression", file: "src/checks/ai.ts",
    from: 'const isListing = declared.has("ItemList");', to: "const isListing = false;" },
